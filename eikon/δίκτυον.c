...
static struct {
    struct { void *cpu; void *gpu; size_t bytes; int est_matrix; int m, n; } t[EIK_GPU_MAX];
    int num;
} gpu_reg;
...
{
    if (gpu_reg.num >= EIK_GPU_MAX) return;
    pfr_matrix_f_t tmp = { m, n, cpu, NULL };
    if (pfr_in_gpu_mitte_f(&tmp) != 0 || !tmp.gpu) return;
    int i = gpu_reg.num++;
    gpu_reg.t[i].cpu = cpu;
...
static void gpu_reg_v(float *cpu, int n)
{
    ...
    if (pfr_in_gpu_mitte_vf(&tmp) != 0 || !tmp.gpu) return;
    int i = gpu_reg.num++;
    gpu_reg.t[i].cpu = cpu;
    gpu_reg.t[i].gpu = tmp.gpu;
...
static void fc_fwd(float *y, const float *W, const float *b,
...
static void fc_bwd_x(float *dx, const float *W, const float *dy,
                     int m, int n)
{
    pfr_matrix_f_t mW  = { m, n, (float *)(uintptr_t)W, gpu_find(W) };
...
static void fc_bwd_w(float *dW, const float *dy, const float *x,
                     int m, int n)
{
    ...
    εικ_σταθμά_t *π, const εικ_σύνθεσις_t *σ, float *δ)
{
    ...
    π->γw1 = p; p += (size_t)σ->γ_κρυ1 * σ->ζ_διαστ;
    π->γb1 = p; p += σ->γ_κρυ1;
    π->γw2 = p; p += (size_t)σ->γ_κρυ2 * σ->γ_κρυ1;
    π->γb2 = p; p += σ->γ_κρυ2;
    π->γw3 = p; p += (size_t)εδ * σ->γ_κρυ2;
    π->γb3 = p; p += εδ;

    π->κw1 = p; p += (size_t)σ->κ_κρυ1 * εδ;
    π->κb1 = p; p += σ->κ_κρυ1;
    π->κw2 = p; p += (size_t)σ->κ_κρυ2 * σ->κ_κρυ1;
    π->κb2 = p; p += σ->κ_κρυ2;
    π->κw3 = p; p += σ->κ_κρυ2;
    π->κb3 = p; p += 1;
...
static int eik_alloc_inf(εικ_t *ε)
{
    ...
    if (!ε->inf_h1 || !ε->inf_h2 || !ε->inf_out) return -1;
    return 0;
}
...
int εικ_ἀνάγνωθι(εικ_t *ε, const char *ὁδός)
{
    ...
    if (!ε->δεδομένα) { fclose(f); return -1; }
